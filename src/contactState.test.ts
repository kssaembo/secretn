import{describe,expect,it}from'vitest';import{canSubmitSeat,chooseTicket,findSeatForDevice,isPlayerAlreadyInRoom,isRoomInUse,submitSeat}from'./contactState';import type{ContactRoom}from'./types';
...
describe('two-tablet contact isolation',()=>{
  ...
  it('rejects the same player on the opposite tablet',()=>{const current=room();expect(isPlayerAlreadyInRoom(current,'p1',1)).toBe(true);expect(isPlayerAlreadyInRoom(current,'p2',1)).toBe(false)});
  it('restores a reconnecting tablet to its previous seat',()=>{const current=room();expect(findSeatForDevice(current,'B')).toBe(1);expect(findSeatForDevice(current,'new-device')).toBe(-1);current.seats[1]={ready:false};expect(findSeatForDevice(current,'new-device')).toBe(1)});
});
